...
function SearchMessages() {
  const [{ currentChatUser, messages, userInfo }, dispatch] = useStateProvider();
  ...
  return (
    <div className="border-conversatio-border border-l w-full bg-conversation-panel-background flex flex-col z-10 max-h-screen">
          ...
          <div className="flex flex-col w-full h-full">
            {searchedMessages.map((message) => {
              ...
              <div className="flex cursor-pointer flex-col justify-center hover:bg-background-default-hover w-full px-5 border-b-[0.1px] border-secondary py-5">
                <div className="text-sm text-secondary">{message.senderId === currentChatUser.id ? currentChatUser.name : userInfo.name} • {calculateTime(message.createdAt)}</div>
